...
export default function Companies({ auth }) {
  ...
  async function load() {
    ...
    try {
      ...
      console.error(e);
      setErrorMsg(e?.message || "Failed to load companies. Please try again.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function add(e) {
    ...
    if (exists) {
      setErrorMsg(`A company with the name "${trimmed}" already exists.`);
      return;
    }
    ...
    try {
      ...
      setName("");
      setSuccessMsg(`Company "${created.name}" created successfully.`);
      setTimeout(() => setSuccessMsg(""), 4000);
    } catch (e) {
      console.error(e);
      setErrorMsg(e?.message || "Failed to create company.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function update(id) {
    ...
    try {
      ...
      console.error(e);
      setErrorMsg(e?.message || "Failed to update company.");
    } finally {
      setSavingEdit(false);
    }
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-slate-500">
                Manage clients and parent organizations for projects.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="rounded-2xl border border-slate-100 bg-slate-50 px-3.5 py-1.5 text-xs font-semibold text-slate-600">
              <span className="font-bold text-slate-900">{items.length}</span> Total Companies
            </div>
            <button
              onClick={load}
              disabled={loading}
      ...
      <div className="grid gap-4 md:grid-cols-12">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:col-span-5">
          <h4 className="mb-3 text-sm font-bold text-slate-900">Search Companies</h4>
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div className="flex items-center gap-2">
            <h4 className="text-base font-bold text-slate-900">Registered Companies</h4>
                  ...
                  <td colSpan={3} className="px-6 py-16 text-center">
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {q
                        ...
                        : "No companies added yet. Register your first company above."}
                    </p>
                  </td>
                </tr>
              )}
